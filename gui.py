...
class PDFFormOCR:
    def __init__(self, root):
        ...
        self.manage_btn = Button(root, text="Manage Fields", command=self.manage_fields)
    ...
    def manage_fields(self):
        if not self.fields:
            messagebox.showinfo("Info", "No fields defined.")
            return

        win = Toplevel(self.root)
        win.title("Manage Fields")

        listbox = Listbox(win)
        listbox.grid(row=0, column=0, columnspan=2, sticky="nsew")

        for field in self.fields:
            listbox.insert("end", field["name"])
        ...
        def rename():
            ...
            new_name = simpledialog.askstring(
                "Rename Field",
                "Enter new name:",
                initialvalue=self.fields[idx]["name"],
            )
            if new_name:
        ...
        def delete():
            ...
            if messagebox.askyesno(
                "Delete Field", f"Delete '{self.fields[idx]['name']}'?"
            ):
                del self.fields[idx]
                listbox.delete(idx)
                self.save_mapping()

        Button(win, text="Rename", command=rename).grid(row=1, column=0, sticky="ew")
        Button(win, text="Delete", command=delete).grid(row=1, column=1, sticky="ew")
